...
def _lab_session(tmp_path, monkeypatch, session_id: str):
    monkeypatch.chdir(tmp_path)
    return SESSION_STORE.load_fixture("lab-aws", session_id=session_id)


def test_search_suggestions_for_k8s_fixture(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    record = SESSION_STORE.load_fixture("k8s-lab", session_id="ui-k8s-suggest")
...
def test_patch_node_properties(tmp_path, monkeypatch):
    record = _lab_session(tmp_path, monkeypatch, "ui-patch-test")
    node_id = next(
...
def test_paths_query_post(tmp_path, monkeypatch):
    record = _lab_session(tmp_path, monkeypatch, "ui-paths-test")
    res = client.post(
...
def test_resolve_start_node_by_arn(tmp_path, monkeypatch):
    record = _lab_session(tmp_path, monkeypatch, "ui-resolve-arn")
...
def test_blast_radius_by_principal_id(tmp_path, monkeypatch):
    record = _lab_session(tmp_path, monkeypatch, "ui-blast-test")
...
def test_import_host_fixture_session(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    res = client.post("/api/sessions/fixtures/host-pivot")
    assert res.status_code == 200
    data = res.json()
    assert data["session_id"]
    assert data["caller_arn"] == "host:workstation:bob-laptop"
    assert data["metadata"]["fixture_id"] == "host-pivot"

    graph = client.get(f"/api/sessions/{data['session_id']}/graph")
    assert graph.status_code == 200
    nodes = graph.json()["nodes"]
    assert any(n.get("resource_type") == "CompromisedHost" for n in nodes)
