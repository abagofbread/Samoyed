...
from samoyed.graph.sample_enterprise import build_sample_enterprise_graph, load_sample_enterprise_session_metadata
...
class SessionStore:
    ...
    def load_sample_enterprise_session(self, session_id: str = "sample-enterprise") -> SessionRecord:
        snapshot = build_sample_enterprise_graph(session_id)
        metadata = load_sample_enterprise_session_metadata()
        metadata["node_count"] = len(snapshot.nodes) - 1
        record = SessionRecord(
            session_id=session_id,
            provider=CloudProvider.AWS,
            caller_arn=metadata["caller_arn"],
...
def is_demo_session(session_id: str, metadata: dict[str, Any] | None = None) -> bool:
    ...
    if metadata.get("sample"):
        return True
    return session_id.startswith("sample-")
